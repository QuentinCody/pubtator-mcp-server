...
export function registerQueryData(server: McpServer, env?: QueryEnv): void {
	const handler = createQueryDataHandler("PUBTATOR_DATA_DO", "pubtator");

	server.registerTool(
		"pubtator_query_data",
		{
			title: "Query Staged PubTator Data",
			description:
				"Query staged PubTator3 data using SQL. Use this when responses are too large and have been staged with a data_access_id.",
			inputSchema: {
